const { Block, Transaction, BlockChain } = require("../models/Blockchain");
...
exports.createNewTransaction = async (req, res, next) => {
  try {
    ...
    const tx = new Transaction(wallet1, wallet2, 100);
...
exports.createNewBlock = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json(
        "block mine successfully!" +
          coin.getBalanceOfAddress(myWallet)
      );
  } catch (err) {
    // next(err);
    res.status(201).json(err.message);
